...
from app.models import TelemetryData, QuarantineLog
...
async def receive_telemetry(data: TelemetryData, pg_db: Session = Depends(get_postgres_db)):
    ...
    save_to_influx(data)
    print(f"Processed: {data.tool_id} | Wafer: {data.wafer_id} | Interlock: {interlock_triggered}")
    return {
        "status": "processed",
...
def get_tool_spc_data(tool_id: str):
    ...
    if not tool_data:
        raise HTTPException(status_code=404, detail="No telemetry found for SPC calculation")
